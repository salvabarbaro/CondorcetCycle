...
res.candidate <- mclapply(ranking.list, FUN = condorcet, runoff = F, quiet = T, mc.cores = 12)  
names(res.candidate) <- caseid.candidate
#
results_candidates.df <- do.call(rbind, 
                                 Map(smr.result.fun, 
                                     as.list(names(res.candidate)), res.candidate))  %>%
  filter(., num_parties > 2) %>% filter(!if_all(everything(), is.na)) 
######  END CANDIDATE PART ####################################################
###############################################################################
...
to.keep <- c("res.candidate", "res.party")
